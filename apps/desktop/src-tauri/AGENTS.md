...
tauri.conf.json        # 单窗口 main 1280x720，identifier com.mineradio.fork.tauri，release CSP 锁定 self + 127.0.0.1 sidecar + AI depth 审核来源
...
- **capabilities 最小集**：当前只授权 renderer core:window/event/webview/app:default；Rust-owned JSON dialogs 和 updater check 都走 Rust commands，不暴露 renderer dialog/updater plugin commands。新功能（shell 或 JS-side fs/dialog/updater）必须**先加 capability 再创命令**，否则 Tauri `invoke` 返 permission denied
