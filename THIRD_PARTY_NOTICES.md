...
| tauri-plugin-updater | 通过（signed-install code-side 接入）；MIT/Apache-2.0 | Tauri updater 检测、签名校验和 signed install 通道 | `tauri.conf.json` 已配置 updater pubkey 并启用 updater artifacts；公开安装更新仍需真实 release manifest、`.sig`、asset 上传和低版本更新安装验证 |
